...
<body id="body">
    ...
    <?php 
      ...
      # Recieve the data from the Support form
      ...
      # Determines which support option was chosen
      switch($sup){
    	case 1: $sup = "Choosing a prebuilt system.";
			break;
	    case 2: $sup = "Finding an upgrade.";
			break;
	    case 3: $sup = "Questions about an order.";
			break;
	    case 4: $sup = "Website troubleshooting.";
			break;
      case 5: $sup = "Other";
      break;
      };

      # Retrieve and rename the uploaded image.  Move it to the support folder.
      if (!empty ($_FILES)){
      $name = $_FILES['filename']['name'];
      $type = substr($name, -4);
      move_uploaded_file($_FILES['filename']['tmp_name'], "../support/".$username.$type);

      $pic = "support/".$username.$type;
      # Create a file with the form data saved with user's email.txt
      $myfile = fopen("../support/".$username.".txt", "w") or die("Unable to open file!");
      $txt = $username."\n".$em."\n".$fn."\n".$ln."\n".$sup."\n".$in."\n".$pic."\n"."Pending Review";
      fwrite($myfile, $txt);
      fclose($myfile);
      } 
    ...
    <center><div>
    <br><br><br><br><br><br>
    <h1>Support Ticket Created</h1><br><h1>An expert will respond within 48 hours.</h1>
