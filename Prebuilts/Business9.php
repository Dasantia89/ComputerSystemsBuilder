...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Ryzen Booster</h1>
          <img class="img-fluid" src="../images/desktopBusiness9.webp" alt="" style="width:30%">
          <br>
          <br>
          <h3 class="textcolor">$600.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="8K39FEFQ4H7JU">
...
<br><br><h3>Average User Rating:</h3>
    ...
    require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 18;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>This computer's Ryzen 7 3700X processor is complemented by the 16GB of high-speed DDR4 RAM, offering better processor performance.</p>
          <p>Tie this together with the 500GB SSD for a boot drive and an additional 1TB of storage, the Ryzen Booster system can handle the even your most rigorous processing needs.</p>
        </div>

        <div style="padding-top: 30px; padding-bottom: 30px;">
          <h2>Specs:</h2>
          <p>Processor: AMD Ryzen 7 3700X</p>
          <p>Graphics: AMD Integrated Graphics</p>
          <p>RAM: 16GB DDR4 3200MHz</p>
          <p>Storage: 500GB SSD / 1TB Hard Drive</p>
          <p>Power Supply: 550W Bronze Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',18,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 18 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
        $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 18;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business9.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 18;");
