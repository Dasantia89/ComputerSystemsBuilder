...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Office Tier 1</h1>
          <img class="img-fluid" src="../images/desktopBusiness1.png" alt="" style="padding-top: 50px">
          <br>
          <br>
          <h3 class="textcolor">$379.00</h3>
        </div><br>
          <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="LZUDTMG7K78PN">
        ...
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 10;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>The perfect entry-level office computer. Sporting an i3 processor and <br>a
            256GB solid-state drive, this starter computer is prefect for simple office needs.
          </p></div>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',10,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 10 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
          $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 10;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business1.php"  enctype="multipart/form-data" ">
        ...
        echo ("<div><h1><strong>User Reviews</strong></h1><hr>"); // Output reviews for this prebuilt
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 10;");
