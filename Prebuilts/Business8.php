...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Ryzen Starter AIO</h1>
          <img class="img-fluid" src="../images/desktopBusiness8.webp" alt="" style="width:30%">
          <br>
          <br>
          <h3 class="textcolor">$500.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="TEEHTSRKCJRJN">
...
<br><br><h3>Average User Rating:</h3>
        ...
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 17;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>Offering the same features as the Ryzen Starter system, this all-in-one computer increases your office space by packing the computer and screen together through one compact design.<p>
          <p>This AMD system offers improved integrated graphics due to AMD's APU technology. Combined with 8GB of high-speed DDR4 RAM and a 500GB SSD,</p>
          <p>This entry-level office computer offers better performance than most other systems in this price range.</p>
        </div>

        <div style="padding-top: 30px; padding-bottom: 30px;">
          <h2>Specs:</h2>
          <p>Processor: AMD Ryzen 5 3400G</p>
          <p>Graphics: AMD Integrated Graphics</p>
          <p>RAM: 8GB DDR4 2666MHz</p>
          <p>Storage: 500GB SSD</p>
          <p>Power Supply: 450W Bronze Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',17,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 17 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
        $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 17;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business8.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 17;");
