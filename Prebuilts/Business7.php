...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Ryzen Starter</h1>
          <img class="img-fluid" src="../images/desktopBusiness7.jpg" alt="" style="width:30%">
          <br>
          <br>
          <h3 class="textcolor">$400.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="PDLWZZ3NHCGWJ">
...
<br><br><h3>Average User Rating:</h3>
       ...
       require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 16;';
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',16,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 16 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
        $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 16;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review 
          echo '<form id="review" method="post" action="Business7.php"  enctype="multipart/form-data" ">
       ...
       $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 16;");
