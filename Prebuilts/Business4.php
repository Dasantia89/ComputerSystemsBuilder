...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Office Tier 3</h1>
          <img class="img-fluid" src="../images/desktopBusiness4.jpg" alt="" style="">
          <br>
          <br>
          <h3 class="textcolor">$650.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="2AGGJADQYHQGW">
...
<br><br><h3>Average User Rating:</h3>
     ...
     require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 13;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>With an i5 processor, 16 GB of high-speed RAM, and 1 TB of solid-state storage, this system is perfectly balanced between performance and value.</p>
          <p>This Tier 3 System outperforms our lower-level models, while offering an increased buffer in RAM and storage, which results in a better user experience.</p>
          <p>NOTE: This system includes a 1-year subscription to Microsoft Office 365!</p>
        </div>

        <div style="padding-top: 30px; padding-bottom: 30px;">
          <h2>Specs:</h2>
          <p>Processor: Intel i5-8400</p>
          <p>Graphics: Intel Integrated Graphics</p>
          <p>RAM: 16GB DDR4 2666MHz</p>
          <p>Storage: 1TB SSD</p>
          <p>Power Supply: 500W Bronze Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',13,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 13 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
        $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 13;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business4.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 13;");
