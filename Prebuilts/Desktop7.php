...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Codename: Velocity</h1>
          <img class="img-fluid" src="../images/slider images/PC7.jpg" alt="" style="padding-top: 50px">
          <br>
          <br>
        ...
        <input type="hidden" name="hosted_button_id" value="63TZRA4M8588U">
...
<br><br><h3>Average User Rating:</h3>
        ...
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 7;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>Really solid mid spec build, this pc utliziing a 2060 super<br> is perfect for gaming at high settings.
          </p>
        </div>
        ...
        <div style="padding-top: 30px; padding-bottom: 30px;">
          ...
          <p>Power Supply: 650W Bronze Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',7,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 7 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
          $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 7;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Desktop7.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 7;");
