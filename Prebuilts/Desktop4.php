...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Codename: VR-Ready</h1>
          <img class="img-fluid" src="../images/slider images/PC4.jpg" alt="" style="padding-top: 50px">
          <br>
          <br>
          <h3 class="textcolor">$1499.00</h3>
        </div><br>
        
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
    <input type="hidden" name="cmd" value="_s-xclick">
    <input type="hidden" name="hosted_button_id" value="NVR6JCT8K6HHL">
...
<br><br><h3>Average User Rating:</h3>
        ...
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 4;';
        ...
        <hr style="border-top: 1px solid #26272b;">
         ...
         <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>Enthusist computer good enough to handle max 1440p gaming.<br> With a 3700x cpu, its perfect for streaming at the same time.<br> Its also a VR ready system that can run any game you throw at it.
          </p>
        </div>
        ...
        <div style="padding-top: 30px; padding-bottom: 30px;">
          ...
          <p>Graphics: Nvidia RTX 2070 Super 8GB</p>
          <p>RAM: 32GB DDR4 3200MHz</p>
          <p>Storage: 1 TB M.2 SSD / 2 TB 7200RPM Hard Drive</p>
          <p>Power Supply: 650W Gold Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',4,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 4 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
          $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 4;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Desktop4.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 4;");
