...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Codename: Midtier</h1>
          <img class="img-fluid" src="../images/slider images/PC2.jpg" alt="" style="padding-top: 50px">
          <br>
          <br>
          <h3 class="textcolor">$850.00</h3>
        </div><br>
        
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="8XXBKSF59C3EW">
        ...
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 2;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>A balanced mid range computer. This PC can do a little of<br> everthing, and is perfect for 1080p gaming.
          </p>
        </div>
        ...
        <div style="padding-top: 30px; padding-bottom: 30px;">
          ...
          <p>Graphics: Nvidia GTX 1660 Ti 6 GB</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',2,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
          $query = ("select * from User_Rating where prebuilt_id = 2 AND user_name = '$username';");
          $result = sendQuery($query);
          if($result == NULL){  //User has not made a rating for this prebuilt 
          $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 2;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
              echo '<form id="review" method="post" action="Desktop2.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 2;");
