...
<html class="no-js">
  ...
  <body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Codename: Icebreaker</h1>
          <img class="img-fluid" src="../images/slider images/PC1.jpg" alt="" style="padding-top: 50px">
          <br>
          <br>
          <h3><strike>$599.00</strike></h3>
          <h3 class="textcolor">$515.00</h3>
        </div><br>

        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="KDWFNW7VPSYEQ">
        <input type="image" src="https://www.sandbox.paypal.com/en_US/i/btn/btn_cart_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
        <img alt="" border="0" src="https://www.sandbox.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
        </form><br><br><h3>Average User Rating:</h3>
        <?php 
        require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 1;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>The perfect entry-level computer to dip<br>
            your feet into the world of PC gaming.
          </p>
        </div>
        <div style="padding-top: 30px; padding-bottom: 30px;">
          ...
          <p>Storage: 240GB SSD / 2TB  7200RPM Hard Drive</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',1,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
          
         
        }else { // Check if user has made a review for this prebuilt
          $query = ("select * from User_Rating where prebuilt_id = 1 AND user_name = '$username';");
          $result = sendQuery($query);
          if($result == NULL){  //User has not made a rating for this prebuilt
            $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 1;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
              echo '<form id="review" method="post" action="Desktop1.php"       enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 1;");
