...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Graphic Pro</h1>
          <img class="img-fluid" src="../images/desktopBusiness6.webp" alt="" style="width:50%">
          <br>
          <br>
          <h3 class="textcolor">$1500.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="UTAM6XZ978ECG">
        <input type="image" src="https://www.sandbox.paypal.com/en_US/i/btn/btn_cart_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
        <img alt="" border="0" src="https://www.sandbox.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
        </form>
<br><br><h3>Average User Rating:</h3>
        <?php 
     require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 15;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>Whether you're just starting out or you're a seasoned veteran, this bundle includes everything a graphic designer needs.</p>
          <p>The Graphic Pro bundle offers an AMD Ryzen 7 3800X processor because Ryzen processors specialize in multi- and hyper-threading, an Nvidia 2060 Super graphics card with 8GB of V-RAM,</p>
          <p>32GB of DDR4 3600 RAM to boost the system's processing performance, and a total of 4TB of SSD storage.</p>
          <p>This bundle also includes a 27-inch 1080p monitor with all of the necessary cables. It's safe to say that the Graphic Pro setup can handle even the most demanding grahpic design needs.</p>
          <p>NOTE: This system includes a 1-year subscription to Microsoft Office 365, Adobe Photoshop, and Adobe Illustrator!</p>
        </div>

        <div style="padding-top: 30px; padding-bottom: 30px;">
          <h2>Specs:</h2>
          <p>Processor: AMD Ryzen 7 3800X</p>
          <p>Graphics: Nvidia GTX 2060 Super 8GB</p>
          <p>RAM: 32GB DDR4 3600MHz</p>
          <p>Storage: 2TB M.2 SSD / 2 TB SSD</p>
          <p>Power Supply: 750W Bronze Certified</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',15,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 15 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
        $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 15;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business6.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 15;");
