...
<body id="body">
    ...
    <section class="section" style="padding-top: 200px">
        <div>
          <h1>Office Tier 1 Bundle</h1>
          <img class="img-fluid" src="../images/desktopBusiness2.jpg" alt="" style="">
          <br>
          <br>
          <h3 class="textcolor">$499.00</h3>
        </div><br>
        <form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
        <input type="hidden" name="hosted_button_id" value="7K6VLHT45MMPL">
...
<br><br><h3>Average User Rating:</h3>
       ...
       require_once '../php/query.php';
        $query = 'select AVG(user_rating) AS avg from User_Rating where prebuilt_id = 11;';
        ...
        <div style="padding-top: 30px">
          <h2>Description:</h2>
          <p>The perfect entry-level office computer. Sporting an i3 processor, 8 GB of RAM,and a 256GB solid-state drive for bootup with a 1TB hard drive for storage, this starter computer is prefect for simple office needs. 
          </p>
          <p>
            This bundle includes the extra 1TB storage drive, a 27-inch monitor for the computer, and all of the necessary cables.
          </p>
        </div>

        <div style="padding-top: 30px; padding-bottom: 30px;">
          <h2>Specs:</h2>
          <p>Processor: Intel i3-8100</p>
          <p>Graphics: Intel Integrated Graphics</p>
          <p>RAM: 8GB DDR4 2133MHz</p>
          <p>Storage: 256GB SSD / 1TB Hard Drive</p>
        ...
        if(isset($_POST['rating'])){ // User has submitted a rating for this prebuilt so it is sent to database
          ...
          $query = ("insert into User_Rating (user_name,prebuilt_id,user_rating,user_review,user_rating_date) values ('$username',11,$rating,'$review',CURRENT_DATE());");
          $result = sendQuery($query);
         
        }else { // Check if user has made a review for this prebuilt
        $query = ("select * from User_Rating where prebuilt_id = 11 AND user_name = '$username';");
        $result = sendQuery($query);
        if($result == NULL){  //User has not made a rating for this prebuilt 
          $query = ("SELECT oi.prebuilt_id from `Order` o JOIN Order_Item oi ON o.order_id = oi.order_id where o.user_name = '$username' AND oi.prebuilt_id = 11;");
            $result = sendQuery($query);
          
            if($result != NULL){ // User owns this prebuilt and is eligible to make a review
          echo '<form id="review" method="post" action="Business2.php"  enctype="multipart/form-data" ">
        ...
        $query = ("select * FROM User u JOIN User_Rating ur ON u.user_name = ur.user_name where ur.prebuilt_id = 11;");
